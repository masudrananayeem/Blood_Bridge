...
const tabs = ["", "pending", "accepted", "completed", "cancelled", "rejected"];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t("admin.manageRequests")}</h1>
        <div className="flex flex-wrap gap-2">
          {tabs.map((tabValue) => (
            <button
              key={tabValue || "all"}
              onClick={() => setSearchParams(tabValue ? { status: tabValue } : {})}
            ...
            <div key={r.id} className="glass-card p-5">
              ...
              {r.acceptedDonorUid && (
                <p className="mb-3 text-xs text-emerald-600">
                  ✓ {t("adminRequests.acceptedBy")} {r.acceptedDonorName} ({r.acceptedDonorPhone})
                </p>
              )}
              {r.isEmergency && (
