...
//            (from "Search Donor"), with an optional personal message.
//         2. isEmergency: true   -> broadcast to only the 5 CLOSEST
//            available donors of the matching blood group (by present
//            address / district distance) — for urgent situations.
//         3. neither             -> normal broadcast to every available
//            donor with the same blood group + district (previous behaviour).
export const createRequest = async (req, res, next) => {
  try {
    ...
    if (targetDonorUid) {
      ...
      notifiedDonorUids = availableDonors(users, req.user.id)
    ...
    const notificationText = targetDonorUid
      ? `${req.user.fullName} সরাসরি আপনাকে ${bloodGroup} রক্তের অনুরোধ পাঠিয়েছেন — ${hospital}, ${district}।${
...
export const getMyRequests = async (req, res, next) => {
  try {
    const requests = (await loadRequests()).filter((request) => request.seekerUid === req.user.id);
    const filtered = req.query.status ? requests.filter((request) => request.status === req.query.status) : requests;
    res.json({ success: true, requests: sortRequests(filtered) });
  } catch (err) {
    next(err);
  }
};
...
export const cancelRequest = async (req, res, next) => {
  try {
    ...
    if (!request || request.seekerUid !== req.user.id) {
      res.status(404);
...
//         this donor — direct requests, emergency nearest-5 picks, and
//         normal blood-group+district broadcasts — minus ones already declined.
export const getIncomingRequests = async (req, res, next) => {
  try {
    const requests = (await loadRequests()).filter(
      (request) =>
        request.status === "pending" &&
        request.seekerUid !== req.user.id &&
        (request.notifiedDonorUids || []).includes(req.user.id) &&
        !(request.rejectedByUids || []).includes(req.user.id)
    );

    res.json({ success: true, requests: sortRequests(requests) });
  } catch (err) {
    next(err);
  }
};
...
//         district that didn't specifically notify them (different blood
//         group, or a target/emergency request aimed at someone else).
export const getNearbyRequests = async (req, res, next) => {
  try {
    const requests = (await loadRequests())
      ...
      .map((request) => ({ ...request, distanceKm: distanceBetweenDistricts(req.user.district, request.district) }))
      .filter((request) => request.distanceKm === null || request.distanceKm <= 100 || request.district === req.user.district);

    res.json({ success: true, requests: sortRequests(requests).slice(0, 20) });
  } catch (err) {
    next(err);
  }
};
...
//         On accept: the seeker's copy of this request now carries the
//         donor's real phone/email (contact is revealed only at this
//         point), the seeker gets a notification, and any other donors who
//         had been notified about the same request are told it's filled.
  ...
  try {
    ...
    if (action === "accept") {
      await requestRef.update({
        status: "accepted",
        acceptedDonorUid: req.user.id,
        acceptedDonorName: req.user.fullName,
        acceptedDonorPhone: req.user.phone,
        acceptedDonorEmail: req.user.email,
        updatedAt: FieldValue.serverTimestamp(),
      });
      ...
      await notify(
        request.seekerUid,
        "accepted",
        `${req.user.fullName} আপনার রক্তের রিকোয়েস্ট গ্রহণ করেছেন — এখন তার সম্পূর্ণ যোগাযোগের তথ্য "My Requests" এ দেখতে পাবেন।`,
        request.id
      );

      // Let any other notified donors know this request has been filled.
      const others = (request.notifiedDonorUids || []).filter((id) => id !== req.user.id);
      await Promise.all(
        others.map((donorId) =>
